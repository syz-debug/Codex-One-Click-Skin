...
try {
  ...
  foreach ($preset in Get-ChildItem -LiteralPath (Join-Path $Root 'presets') -Directory) {
    & $node.Path (Join-Path $Root 'scripts\injector.mjs') --check-payload `
      --theme-dir $preset.FullName *> $null
    if ($LASTEXITCODE -ne 0) { throw "Theme payload validation failed: $($preset.Name)" }
  }
