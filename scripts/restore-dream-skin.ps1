...
try {
  ...
  $StateRoot = Join-Path $env:LOCALAPPDATA 'CodexOneClickSkin'
  ...
  try {
    ...
    if ($Uninstall) {
      ...
      @(
      (Join-Path $desktop 'Codex 一键换肤.lnk'),
      (Join-Path $desktop 'Codex 一键换肤 - 启动.lnk'),
      (Join-Path $desktop 'Codex 一键换肤 - 恢复原生.lnk'),
      (Join-Path $startMenu 'Codex 一键换肤.lnk'),
      (Join-Path $startMenu 'Codex 一键换肤 - 启动.lnk')
  ...
  Write-Host 'Codex One-Click Skin restore completed; any saved CDP session was closed.'
} finally {
