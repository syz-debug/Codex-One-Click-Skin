...
try {
  ...
  if ($ThemeId) {
    $ThemeDirectory = Set-OneClickSkinSelectedTheme -ThemeId $ThemeId
  } else {
    $ThemeId = Get-OneClickSkinSelectedThemeId
    $ThemeDirectory = Get-OneClickSkinThemeDirectory -ThemeId $ThemeId
  }
  $LibraryRoot = Join-Path $StateRoot 'themes'
  $manifest = Get-OneClickSkinThemeManifest -ThemeDirectory $ThemeDirectory
  if ("$($manifest.id)" -cne $ThemeId) { throw "Selected theme manifest id does not match: $ThemeId" }
  ...
  if ($ForegroundInjector) {
    ...
    & $node.Path $Injector --watch --port $Port --browser-id $cdpIdentity.BrowserId `
      --theme-dir $ThemeDirectory --library-dir $LibraryRoot
    exit $LASTEXITCODE
  }
  ...
  try {
    $injectorArgs = @((ConvertTo-DreamSkinProcessArgument -Value $Injector), '--watch', '--port', "$Port",
      ...
      (ConvertTo-DreamSkinProcessArgument -Value $ThemeDirectory), '--library-dir',
      (ConvertTo-DreamSkinProcessArgument -Value $LibraryRoot))
  ...
  Write-Host "Codex One-Click Skin '$($manifest.name)' is active on verified loopback port $Port."
} finally {
